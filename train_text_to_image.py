...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "It will upload the models to HuggingFace every X epochs"
        ),
    )
    parser.add_argument(
        "--dataset_from_disk",
...
def main():
    ...
    # Read validations prompts
    with open("./finetuning_validation.txt") as f:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            ...
            # run pipeline in inference with some requests
            n_samples, scale, steps = 1, 7.5, 50
            pipeline.to("cuda:0")
            for prompt in prompts:
                os.makedirs(f"{args.output_dir}/data/results/{prompt}", exist_ok=True)
                images = pipeline(n_samples*[prompt], guidance_scale=scale, num_inference_steps=steps).images
                for idx, im in enumerate(images):
                    im.save(f"{args.output_dir}/data/results/{prompt}/{prompt}_{epoch}_{idx:06}.png")

            if epoch % args.upload_every_epoch == 0:
